...
namespace WpfHuaWei.DeviceView {
    /// <summary>
    /// CurrentJiHutao.xaml的交互逻辑
    /// </summary>
    public partial class CurrentJiHutao : BaseMachine {
        /// <summary>
        /// 用于更新界面上的当前时间
        /// </summary>
        private DispatcherTimer timer = new DispatcherTimer();

        public CurrentJiHutao() {
            InitializeComponent();

            timer.Interval = TimeSpan.FromMilliseconds(995);
            timer.Tick += timer_Tick;

            this.Loaded += CurrentJiHutao_Loaded;
        }

        private void CurrentJiHutao_Loaded(object sender, RoutedEventArgs e) {
            timer.Start();

            // Issue:如果设置pdfReader可见，初始化后diameterChart就看不见
            this.pdfReader.Visibility = System.Windows.Visibility.Visible;
        }
        ...
        private void tbQrcode_TextChanged(object sender, TextChangedEventArgs e) {
            // 获取工单号
            string worksheetNo = tbQrcode.Text.Trim();
            if (!isWorksheetNo(tbQrcode.Text)) {
                // 长度超过了一个编码长度，则清空文本
                if (worksheetNo.Length > simpleWorkSheetNo.Length) {
                    tbQrcode.Text = string.Empty;
                }
                return;
            }
            Task.Run(() => {
                updateWorkSheetInfo(worksheetNo);
            });
            e.Handled = true;
        }
